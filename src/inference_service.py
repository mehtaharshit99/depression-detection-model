"""
inference_service.py — Runtime Inference Service
=================================================
Transcript text is cleaned with clean_transcript_text() /
clean_participant_utterances() before embedding, matching training.
"""
...
import torch.nn as nn  # noqa: F811 (shadow is intentional for type hint)

BASE_DIR = Path(__file__).resolve().parents[1]
try:
    from src.pipeline_utils import (
        CHUNK_SEC, TARGET_SR, chunk_waveform,
        MultimodalGRUSequenceClassifier,
        clean_participant_utterances,
...
@lru_cache(maxsize=1)
def load_artifacts():
    """Load all reusable inference artifacts once per server process."""
    processor = Wav2Vec2Processor.from_pretrained(
        "facebook/wav2vec2-base-960h", cache_dir=HF_CACHE
    )
    ...
    if SCALER_PATH.exists():
        ...
        if not feature_files:
            raise FileNotFoundError(
                f"No scaler at {SCALER_PATH} and no feature CSVs in {FEATURE_DIR}."
            )
        feature_cols = []
        for fp in feature_files:
            df   = pd.read_csv(fp)
            wcol = sorted([c for c in df.columns if c.startswith("w2v_")],  key=lambda c: int(c.split("_")[1]))
            tcol = sorted([c for c in df.columns if c.startswith("text_")], key=lambda c: int(c.split("_")[1]))
            feature_cols = wcol + tcol
            scaler.partial_fit(df[feature_cols].values.astype(np.float32))
        audio_dim = 768
        text_dim  = max(0, len(feature_cols) - audio_dim)

    model_paths = sorted(MODEL_DIR.glob("best_multimodal_concat_fold*.pt"))
    if not model_paths:
        raise FileNotFoundError(
            f"No trained models found in {MODEL_DIR}. Run 03_train_sequence.py first."
        )
...
def load_sequence_model(
    ...
    kwargs = dict(input_dim=audio_dim, text_dim=text_dim, hidden_dim=128, num_layers=2, dropout=0.35)
    model: nn.Module = MultimodalGRUSequenceClassifier(**kwargs)
...
def load_transcript_bytes(raw_bytes: bytes) -> pd.DataFrame:
    """Parse an uploaded transcript CSV from raw bytes."""
    ...
    if "start_timestop_timespeakervalue" in first_line.replace(" ", "").lower():
        transcript = pd.read_csv(
            io.StringIO(raw_text), sep="\t", skiprows=1,
    ...
    if "speaker" not in transcript.columns:
        collapsed = "".join(transcript.columns).replace(" ", "")
        if "start_timestop_timespeakervalue" in collapsed:
            transcript = pd.read_csv(
                io.StringIO(raw_text), sep="\t", skiprows=1,
                names=["start_time", "stop_time", "speaker", "value"],
            )
...
def build_participant_text(transcript: pd.DataFrame) -> str:
    """
    extract and clean participant utterances before embedding.
    Uses the same clean_participant_utterances() function as training extraction
    ...
    # apply cleaning
    return clean_participant_utterances(participant_rows)
...
def extract_text_embedding(
    text: str, text_tokenizer, text_model, text_dim: int = TEXT_DIM
) -> np.ndarray:
    """text is already cleaned before reaching this function."""
    if text_dim <= 0:
...
def isolate_participant_audio(
    waveform: np.ndarray, sr: int, transcript: pd.DataFrame
) -> np.ndarray:
...
def extract_chunk_embedding(
    chunk: np.ndarray, processor, wav2vec2
) -> np.ndarray | None:
    rms = np.sqrt(np.mean(chunk ** 2))
    if rms < 1e-4:
        return None

    inputs       = processor(chunk, sampling_rate=TARGET_SR, return_tensors="pt", padding=False)
...
def run_inference(
    ...
    text_features     = (
        ...
        if text_dim > 0 else np.zeros(0, dtype=np.float32)
    )
    ...
    for model_path in model_paths:
        mdl  = load_sequence_model(model_path, audio_dim=audio_dim, text_dim=text_dim)
        logit = mdl(x, text_x, lengths)
    ...
    return {
        ...
        "confidence_band": (
            "high"     if abs(final_prob - threshold) >= 0.20 else
            "moderate" if abs(final_prob - threshold) >= 0.10 else
            "low"
        ),
...
def predict_from_upload(
    ...
    """Complete upload-to-prediction inference workflow."""
    processor, wav2vec2, text_tokenizer, text_model, scaler, audio_dim, text_dim, model_paths = (
        load_artifacts()
    )
    ...
    if transcript_bytes:
        ...
        # build_participant_text now cleans with clean_participant_utterances()
